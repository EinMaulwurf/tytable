...
Use `.images()` to embed existing files without Matplotlib or the optional `images` extra. This standalone example copies three committed SVG country flags into the default `build/07_static_images_assets/` directory, making the generated fragment and its sibling assets portable together:
...
See #link(<saving-typst>)[Saving and using a table in Typst] for the asset directories, copy/embed/reference policies, and the difference between `.render()` and `.save()` for media.
== Alternative backends <alternative-backends>

Typst is the primary output format, but a `TyTable` can render the same recorded intent as HTML or fixed-width terminal text. These backends are useful for development, tests, and applications that do not need a paged document. They are independent renderers, not conversions of the generated Typst, so use a compiled Typst preview for final layout decisions.
...
The renderers share recorded style intent, but only apply properties their output medium can represent. In the matrix below, “yes” means the property is rendered, “plain” means the content is retained without that styling, and “—” means it has no meaningful representation.
...
#docs-table(
  ...
  [yes],
  [plain],
  [Caption: background, align, indent],
  [—],
  [yes],
  [—],
  [Notes: font styles, color, size],
  [yes #super[1]],
  [yes],
  [plain],
  [Notes: background, align, indent],
  [yes],
  [yes],
  [—],
)
...
#text(size: 8.5pt)[
  #super[1] Typst caption and note text supports bold, italic, underline, strikeout, small caps, color, and size; HTML additionally supports monospace. Typst notes, unlike captions, also support background and horizontal/vertical alignment. Cell font styles include bold, italic, underline, strikeout, monospace, and small caps in both visual backends.
]

Caption and note styling is validated when that backend renders. If a directive contains a property the selected Typst or HTML backend cannot apply, rendering raises a targeted `ValueError`; use `output=("typst",)` or `output=("html",)` for intentionally backend-specific styling. ASCII always emits captions and notes as plain text and ignores their style properties. Typst-only color constructors likewise require `output=("typst",)`; portable named and hex colors work in both visual backends.
