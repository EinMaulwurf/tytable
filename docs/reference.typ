...
Non-negative integer `i` values range from zero through the source DataFrame height minus one. Row-group separators never change what an integer selects. `"header"` is empty when column names are hidden, and `"groupj"` is empty when no column-group rows exist. Import `groupi`, `rowgroup`, and `groupj` from `tytable.selectors` for typed group selectors. Group labels may be strings or other displayable values; registration and typed selectors normalize them to nonempty strings. `groupi()` selects every row-group separator, while `groupi(label="A")` selects every separator whose original registered label is exactly `"A"`. `rowgroup(label="A")` selects the source-data rows after each matching separator and before the next separator. Repeated labels all match, later formatting of the displayed label does not affect selection, and a missing label raises `ValueError`; a separator after the final source row has an empty member selection. Use `groupj(level=n)` with `.style()` to select one nested column-group header level. Level 0 is the first-created, innermost level nearest the ordinary column names; every later `.group(j=...)` call creates the next outer level above it. Levels retain these identities when `.show_columns()` removes an empty header level. Lists and tuples may mix typed group selectors with integer and string row selectors. `.style()` also accepts `i="caption"` and `i="notes"`, either directly or inside a mixed selector sequence; these non-grid targets allow only their documented text-oriented properties.
